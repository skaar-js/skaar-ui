 ...
 * @type {symbol}
 */
export const BREAK = global._X_LOOP_BREAK_;
/**
 * Match Any: Used as value in predicate object
 * @type {symbol}
 */
export const ANY = global._X_ANY_;
export const ALL = global._X_ALL_;
 ...
 * @returns {boolean}
 */
export function contains(src, value, key) {
...
export function objMatchOne(o, match) {
...
export function objMatchAll(o, match) {
    let m = Object.keys(match);
    for (let k of m) {
...
export function predicate(f, def = () => true, inc = true) {
 ...
 * @param {Number} [step]
 */
export function forN(func, init=0, finish=0, step=init<finish?1:-1) {
    if (step>0) {
 ...
 * @returns {*|number}
 */
export function forEachRange(src, func, start = 0, end) {
 ...
 * @returns {number|*}
 */
export function forEachRight(src, func, range = []) {
    if (!T.isArr(src) || !T.isStr(src)) {
        let i = 0;
        let keys = Object.keys(src);
 ...
 * @returns {number}
 */
export function firstIndex(src, pred) {
 ...
 * @returns {*}
 */
export function first(src, pred) {
 ...
 * @returns {boolean|*}
 */
export function startsWith(src, pred) {
 ...
 * @returns {number}
 */
export function lastIndex(src, pred) {
    pred = predicate(pred, () => true);
    let r = -1;
 ...
 * @returns {boolean|*}
 */
export function endsWith(src, pred) {
    if (T.isStr(src) && T.isStr(pred)) {
...
export function filterStr(src, pred, right = false, omit = false) {
    pred = predicate(pred, () => true);
    let res = "";
...
export function filterObj(src, pred, right = false, omit = false) {
    if (T.isArr(pred)) {
...
export function filterArr(src, pred, right = false, omit = false) {
    if (T.isArr(pred)) {
        let a = Object.assign([], pred)
 ...
 */
export function filter(src, pred, right = false) {
 ...
 * @returns {Array|Object|String}
 */
export function omit(src, pred, right = false) {
 ...
 * @returns {Array|Object|String}
 */
export function filterRight(src, pred) {
...
export function mapArr(src, func, right = false) {
    let res = [];
    // let loop = right ? ForEachRight : ForEach;
    const len = src.length;
    if (!right) {
...
export function mapObj(src, func, right = false) {
    let res = {};
    // let loop = right ? ForEachRight : ForEach;
    const keys = Object.keys(src);
    const len = src.length;
 ...
 * @returns {Array|Object|String}
 */
export function map(src, transform, right = false) {
 ...
 * @returns {any}
 */
export function reduce(src, func, acc = src) {
    if (T.isUnd(func)) {
        func = (rs, v) => rs + v
    }
    forEach(src, (v, k, src) => {
 ...
 * @returns {any}
 */
export function reduceRight(src, func, acc = src) {
 ...
 * @return {Object[]}
 */
export function keyValuePairs(object) {
    let entries = [];
    forEach(object, (v, k) => {
 ...
 */
export function translateObject(source, dict) {
    if (T.isArr(dict)) {
        filter(source, dict);
    }
...
export function _keyGen(key, fnName) {
    let keyGen = key
    if (T.isStr(key)) {
